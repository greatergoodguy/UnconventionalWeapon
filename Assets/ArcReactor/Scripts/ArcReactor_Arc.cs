...
public class ArcReactor_Arc : MonoBehaviour {
	...
	void LateUpdate () 
	{
		...
		for (int n = 0; n < arcs.Length; n++)
		{
			...
			{
			case FadeTypes.none:
				lrends[n].material.SetFloat("_FadeLevel",0);
				break;
			case FadeTypes.relativePoint:
				lrends[n].material.SetFloat("_FadeLevel",arcs[n].colorOptions.fadePoint);
				break;
			case FadeTypes.worldspacePoint:
				lrends[n].material.SetFloat("_FadeLevel",Mathf.Clamp01(arcs[n].colorOptions.fadePoint / shapeLength));
				break;
			}
